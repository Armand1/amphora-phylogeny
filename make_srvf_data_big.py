#!/usr/bin/env python3
"""make_srvf_data_big.py -- SRVF tangent-PCA characters + ages for the 77-taxon set.
Self-contained sibling of make_srvf_data.py (reads selected_taxa_big.csv, writes both
the NEXUS and the taxa TSV). Same fdasrsf settings (mode='O', no scale/rotation).
Run: python3 make_srvf_data_big.py 38
"""
...
sel  = pd.read_csv(f"{HERE}/selected_taxa_big.csv")
...
nex = f"{HERE}/data/mini_character_big_srvf_{N_PC}.nex"
pad = max(len(t) for t in types) + 4
with open(nex, "w") as f:
    f.write("#NEXUS\n[SRVF tPCA 77-taxon, make_srvf_data_big.py]\nBEGIN DATA;\n")
...
ages[["type","min_age","max_age"]].rename(columns={"type":"taxon"}).to_csv(
    f"{HERE}/data/mini_taxa_big.tsv", sep="\t", index=False)
print(f"wrote {os.path.basename(nex)} (NTAX={len(types)} NCHAR={N_PC}) + mini_taxa_big.tsv")
